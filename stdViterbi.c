...
void traceback()
{
    ...
    if (s_col != NULL && s_col->next != NULL)
    {
        s_col->next->previous = NULL;
    }
    if (p_col != NULL && p_col->next != NULL)
    {
        p_col->next->previous = NULL;
    }
    ...
    while (depth-- > 0)
    { //Traceback from new root to previous root
